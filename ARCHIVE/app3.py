...
# ---- logs paths ----
LOGS_DIR = (Path(__file__).resolve().parent).resolve()
SESSIONS_DIR = os.path.join(LOGS_DIR, "logs","sessions", "messages.json")
#from pathlib import Path

#SESSIONS_DIR = (Path(__file__).resolve().parent).resolve()
...
def _find_messages_file():
    candidates = [
        os.path.join(LOGS_DIR, "messages.json"),
    ]
    logging.info("Sessions: BASE_DIR=%s LOGS_DIR=%s", BASE_DIR, LOGS_DIR)
    for p in candidates:
        logging.info("Sessions: check %s exists=%s", p, os.path.exists(p))
        if os.path.exists(p):
            return p
    for d in {LOGS_DIR, os.path.join(LOGS_DIR, "sessions")}:
        try:
            listing = os.listdir(d)
        except Exception as e:
            listing = f"error: {e}"
        logging.info("Sessions: listdir(%s) -> %s", d, listing)
    return None

def load_messages_with_logging():
    path = find_messages_file()
    msgs, err = [], None
    if not path:
        err = "messages.json not found in expected locations"
        logging.warning("Sessions: %s", err)
        return msgs, err, None

    try:
        with open(path, "r", encoding="utf-8") as f:
            raw = f.read()
        logging.info("Sessions: read %d bytes from %s", len(raw), path)
        try:
            data = json.loads(raw)
            msgs = data.get("messages", data if isinstance(data, list) else [])
            logging.info("Sessions: parsed %d messages from JSON object/list", len(msgs))
        except json.JSONDecodeError:
            # JSONL fallback
            msgs = [json.loads(line) for line in raw.splitlines() if line.strip()]
            logging.info("Sessions: parsed %d messages from JSONL", len(msgs))
    except Exception as e:
        err = f"read/parse error: {e}"
        logging.exception("Sessions: %s", err)
...
def _load_messages():
    ...
    try:
        ...
        try:
            data = json.loads(raw)
            msgs = data.get("messages", data if isinstance(data, list) else [])
            logging.info("Sessions: parsed %d messages (JSON object/list)", len(msgs))
        except json.JSONDecodeError:
            msgs = [json.loads(line) for line in raw.splitlines() if line.strip()]
            logging.info("Sessions: parsed %d messages (JSONL)", len(msgs))
    except Exception as e:
        err = f"read/parse error: {e}"
        logging.exception("Sessions: %s", err)
    return msgs, err, path
...
def sessions_html():
    logger.info(f"/sessions.html list {SESSIONS_DIR}")
    try:
        files = sorted(
            [p.name for p in SESSIONS_DIR.iterdir() if p.is_file()],
            key=lambda n: (SESSIONS_DIR / n).stat().st_mtime,
            reverse=True,
        )
    except FileNotFoundError:
        logger.warning("missing %s", SESSIONS_DIR)
        files = []
    return render_template("sessions.html", files=files, home_url="/")

@app.get("/logs/sessions/<path:filename>")
def serve_session_file(filename):
    logger.info(f"serve {filename}")
    return send_from_directory(str(SESSIONS_DIR), filename)
